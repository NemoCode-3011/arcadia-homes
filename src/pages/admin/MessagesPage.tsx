...
import { MapPin, Clock, ChevronRight, X, UserPlus } from "lucide-react"
...
const MessagesPage = () => {
  ...
  const unreadProperty = messageList.filter((m) => !m.isRead).length
  ...
  return (
    <div className="p-3 lg:p-6 space-y-6">
        ...
        <div className="space-y-4">
          {messageList.map((message) => (
            <div
              ...
              className={`rounded-xl p-6 flex items-center justify-between transition-all duration-300 border cursor-pointer ${
                !message.isRead
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`w-3 h-3 rounded-full mt-2 shrink-0 ${
                  message.isRead ? "bg-arcadia-moss" : "bg-red-500"
                }`} />
                <div className="w-10 h-10 rounded-full bg-arcadia-moss flex items-center justify-center text-arcadia-cream font-boldshrink-0">
      ...
      {activeTab === "general" && (
        <div className="space-y-4">
          {generalList.map((enquiry) => (
            <div
              key={enquiry.id}
              className={`rounded-xl p-6 flex items-center justify-between transition-all duration-300 border ${
                !enquiry.isRead
                  ? "bg-arcadia-bark/20 border-red-400"
                  : "border-arcadia-bark"
              }`}
            >
              <div className="flex items-start gap-4 cursor-pointer flex-1" onClick={() => setSelectedMessage({ ...enquiry, type: "general" })}>
                <div className={`w-3 h-3 rounded-full mt-2 shrink-0 ${
                  enquiry.isRead ? "bg-arcadia-moss" : "bg-red-500"
                }`} />
              ...
              <div className="flex items-center gap-4">
                ...
                {enquiry.assignedAgentId ? (
                  ...
                  <select
                    onChange={(e) => {
                      if (e.target.value) assignAgent(enquiry.id, e.target.value)
                    }}
                    defaultValue=""
      ...
      {/* Detail Modal — works for both types */}
      {selectedMessage && (
        <div
